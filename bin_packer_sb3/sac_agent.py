...
class DenseActor(BasePolicy):
    ...
    def __init__(
        ...
        if self.use_sde:
            ...
            self.mu, self.log_std = self.action_dist.proba_distribution_net(
                latent_dim=features_dim, latent_sde_dim=latent_sde_dim, log_std_init=log_std_init
            )
            ...
            self.mu = nn.Linear(features_dim, action_dim)
            self.log_std = nn.Linear(features_dim, action_dim)
    ...
    def get_action_dist_params(self, state: th.Tensor) -> Tuple[th.Tensor, th.Tensor, Dict[str, th.Tensor]]:
        """
...
class BinPacker_ACPolicy_SAC(SACPolicy):
    """
    ...
    def __init__(self, *args, **kwargs):
        super(BinPacker_ACPolicy_SAC, self).__init__(
            *args,
            **kwargs,
        )
...
class BinPackerFeatureExtractor(BaseFeaturesExtractor):
    def __init__(self, observation_space, features_dim = 512):
        super().__init__(observation_space, features_dim)
        n_input_channels = observation_space.shape[0]
        self.backbone = nn.Sequential(
            nn.Conv2d(n_input_channels, 5, kernel_size=5, stride=2, padding=0, bias=True),
            nn.BatchNorm2d(5),
            nn.ReLU(),
            nn.Conv2d(5, 10, kernel_size=5, stride=2, padding=0, bias=True),
            nn.BatchNorm2d(10),
            nn.ReLU(),
            nn.Conv2d(10, 20, kernel_size=5, stride=2, padding=0, bias=True),
            nn.BatchNorm2d(20),
            nn.ReLU(),
            nn.Flatten()
        )
        ...
        with th.no_grad():
            n_flatten = self.backbone(th.as_tensor(observation_space.sample()[None]).float()).shape[1]
    ...
    def forward(self, observations):
        return self.linear(self.backbone(observations))
